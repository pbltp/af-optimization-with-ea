...
The network weights $W$ are initialized once at the start and are only updated when the best individual of a generation improves upon the current champion $C$, in which case one mini-batch training step is performed using $B_b$.
...
\STATE Set champion $C \leftarrow \arg\max_{f \in P^0} fitness(f)$
\FOR{$t = 0$ to $g$}
    \FOR{$i = 1$ to $M$}
    ...
    \IF{$fitness(\arg\max_{f \in P^{t+1}} fitness(f)) > fitness(C)$}
        \STATE $C \leftarrow \arg\max_{f \in P^{t+1}} fitness(f)$
        \STATE Train $W$ for one step on $B_b$
    \ENDIF
