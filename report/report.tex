...
For simplicity, we assume here that the neural network has one hidden layer,
but an extension to multiple layers is possible.

First, the initial generation is chosen as a random list of functions.
Then we perform a training of the resulting network with a fixed number of
steps.  Of these networks, the one with the best fitness is kept.
